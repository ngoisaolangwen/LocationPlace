...
public class FoodGoogleMap extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_food_google_map);

		/* Hiá»ƒn thá»‹ map khi Activity dc gá»�i. */
		onLoadGoogleMap();
		Bundle bundle = getIntent().getExtras();
		boolean foodIntent = bundle.getBoolean("food");
		/* áº¨n MenuBar */
		menuBarHide();
		/* Náº¿u gá»�i Ä‘á»ƒ tÃ¬m Ä‘Æ°á»�ng Ä‘i. */
		if (foodIntent) {
			/* Show Progress Dialog. */
			showDialog(0);
			/* TÃ¬m Ä‘Æ°á»�ng. */
			new DownloadTask().execute();
		} else {/* Gá»�i Ä‘á»ƒ láº¥y vá»‹ trÃ­ thÃªm báº¥t ká»³. */
			
			/* Di chuyá»ƒn Ä‘áº¿n vá»‹ trÃ­ ngÆ°á»�i dÃ¹ng Ä‘ang Ä‘á»©ng. */

			_myMap.moveCamera(CameraUpdateFactory.newLatLng(new LatLng(
			...
			_myMap.setOnMapLongClickListener(new OnMapLongClickListener() {
				...
				public void onMapLongClick(LatLng arg0) {
					...
					/* Intent gá»�i class AddPlace.java */
					Intent intent;
					intent = new Intent(FoodGoogleMap.this, AddFood.class);

					Bundle bundle = new Bundle();

					/* Truyá»�n vÃ o tá»�a Ä‘á»™ cá»§a ng dÃ¹ng cháº¡m vÃ o map. */
					String lat = Double.toString(arg0.latitude);
					String lng = Double.toString(arg0.longitude);
	...
	private void onLoadGoogleMap() {
		...
		_myMap.setInfoWindowAdapter(new InfoWindowAdapter() {
			...
			public View getInfoContents(Marker arg0) {
				...
				View v = getLayoutInflater().inflate(R.layout.item_marker_info,
						null);
	...
	/** Add Marker for GoogleMap */
	public void addMarker(MyFood myFood) {
		// Creat markerOption
		MarkerOptions markerOption = new MarkerOptions();
		double _lat = Double.parseDouble(myFood.get_lat());
		double _lng = Double.parseDouble(myFood.get_lng());
		LatLng point = new LatLng(_lat, _lng);
		markerOption.position(point);
		String info = myFood.get_address() + ", PhÆ°á»�ng " + myFood.get_ward()
				+ ", " + myFood.get_district();
		markerOption.snippet(info).title(myFood.get_name());
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.food_google_map, menu);
		return true;
	}
	...
	private class DownloadTask extends AsyncTask<Void, Void, PolylineOptions> {
		...
		protected PolylineOptions doInBackground(Void... url) {
			...
			double lat = 0, lng = 0;
			LatLng pointLocal = new LatLng(MainLocationPlace._getMyLocation.getLatitude(),
					MainLocationPlace._getMyLocation.getLongitude());
			try {

				lat = Double.parseDouble(MainLocationPlace._food.get_lat());
				lng = Double.parseDouble(MainLocationPlace._food.get_lng());
		...
		protected void onPostExecute(PolylineOptions result) {
			...
			addMarker(MainLocationPlace._food);
