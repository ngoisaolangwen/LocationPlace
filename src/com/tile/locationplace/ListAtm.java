...
public class ListAtm extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_list_atm);
		
		/* Láº¥y má»™t Intent vá»�. */
		atm = getIntent();
		String str = atm.getStringExtra("nameId");
		Log.d("nameId", str);
		/* Khá»Ÿi táº¡o cÃ¡c biáº¿n. */
		myAtmDB = getAtmNameId(MainLocationPlace.arrayAtm, str);
		...
		/* Hide MenuBar. */
		menuBarHide();
		lv = (ListView)findViewById(R.id.lv_atm);
	...
	private ArrayList<MyATM> getAtmNameId(ArrayList<MyATM> listAtm, String str){
		
		ArrayList<MyATM> list = new ArrayList<MyATM>();
		for(MyATM atm : listAtm){
			if(atm.get_Nameid().equalsIgnoreCase(str)){
				list.add(atm);
			}
		}
		return list;
...
//		actionB.setDisplayShowHomeEnabled(false);
		...
		mQ.setOnActionItemClickListener(new OnActionItemClickListener() {
			...
			public void onItemClick(QuickAction source, int pos, int actionId) {
				...
				/* Váº½ Ä�Æ°á»�ng Ä‘i Ä‘áº¿n Ä‘iá»ƒm Ä‘Æ°á»£c chá»�n. */
					Intent timDuong = new Intent(ListAtm.this,AtmGoogleMap.class);
	...
	private void Location(){
		...
		// custom dialog
		final Dialog dialog = new Dialog(ListAtm.this);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.list_atm, menu);
		return true;
	}
...
//					Log.d("KM", distance);
			...
			for (int i = 0; i < myAtmTam.size() - 1; i++){
		...
		// Executes in UI thread, after the execution of
				...
				protected void onPostExecute(List<MyATM> result) {
					...
					lv.setAdapter(new AdapterATM_Near(getApplicationContext(),
							R.layout.activity_list_atm, result));
					Log.d("size", result.size()+"");
					if(result.size() <0){
						Toast.makeText(ListAtm.this,
